...
class RepertoryEngine {
  ...
  static Future<List<RubricResult>> searchSymptom(String rawQuery) async {
    ...
    for (final token in keywords) {
      whereClauses.add(
        '(r2.full_path LIKE ? OR r2.rubric_text LIKE ? OR c2.name LIKE ? OR (pc2.name IS NOT NULL AND pc2.name LIKE ?))',
      );
    ...
    final sql = '''
      ...
      WHERE r.id IN (
        SELECT r2.id
        FROM rubrics r2
        INNER JOIN chapters c2 ON c2.id = r2.chapter_id
        LEFT JOIN chapters pc2 ON pc2.id = c2.parent_chapter_id
        WHERE ${whereClauses.join(' AND ')}
        ORDER BY r2.page_number ASC, r2.id ASC
        LIMIT 60
      )
